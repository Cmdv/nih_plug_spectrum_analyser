...
impl<Message> Program<Message, Theme> for GainKnob {
    type State = ();
...
impl GainKnob {
    ...
    fn draw_value_text(&self, frame: &mut Frame, size: Size, center: Point, gain_db: f32) {
        ...
        let bg_rect = Path::rectangle(
            Point::new(center.x - indicator_width/2.0, indicator_y - indicator_height/2.0),
        ...
        // Value indicator dot
        let value_dot = Path::circle(
            Point::new(value_x, indicator_y),
            2.0
        );
